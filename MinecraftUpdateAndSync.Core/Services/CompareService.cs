...
namespace MinecraftUpdateAndSync.Core.Services
{
    public class CompareService
    {
        ...
        public static CompareResult? CompareFile(FileSnapshot localFile, ManifestFile remoteFile)
        {
            ...
            {
                RelativePath = remoteFile.RelativePath ?? string.Empty,
            };
            ...
            else if (string.IsNullOrEmpty(localFile.Hash))
            {
                LogHelper.LogError($"Failed to compute local file hash.", LogHelper.LogDebugLevel.None);
                result.State = FileState.InvalidLocal;
                return result;
            }
            ...
            Dictionary<string, FileSnapshot> localFiles)
        {
            ...
            foreach (var remoteFile in manifest.Files)
            {
                ...
                { 
                    if (localFiles.TryGetValue(remoteFile.RelativePath, out var localFile))
                    {
                ...
                { 
                    LogHelper.LogError($"Remote file has null or empty relative path, skipping this file: {remoteFile.RelativePath ?? ""}", LogHelper.LogDebugLevel.None);
                    nullCounts++;
                }
            }
            if (nullCounts > 0) LogHelper.LogError($"There are {nullCounts} files with null or empty relative path in the manifest, which are skipped in the compare process.", LogHelper.LogDebugLevel.None);
            ...
            Dictionary<string, FileSnapshot> localFiles)
        {
            ...
            foreach (var remoteFile in deletedManifest.Files)
            {
                ...
                {
                    LogHelper.LogError($"Remote file has null or empty relative path, skipping this file.", LogHelper.LogDebugLevel.None);
                    nullCounts++;
                }
            }
            if (nullCounts > 0) LogHelper.LogError($"There are {nullCounts} files with null or empty relative path in the deleted manifest, which are skipped in the compare process.", LogHelper.LogDebugLevel.None);
            return compareResults;
        }
        ...
        public static List<CompareResult> CompareManifests(Manifest manifest1, Manifest manifest2)
        {
            ...
            Dictionary<string, ManifestFile> oldManifestDict;
            Dictionary<string, ManifestFile> newManifestDict;
            ...
            {
                var result = new CompareResult();
                if (oldManifestDict.TryGetValue(newFile.RelativePath, out var manifest2File))
                {
            ...
            {
                if (!newManifestDict.TryGetValue(oldFile.RelativePath, out var manifest2File))
                {
